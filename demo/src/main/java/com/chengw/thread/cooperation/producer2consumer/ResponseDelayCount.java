...
import com.chengw.thread.cooperation.producer2consumer.common.Config;
import com.chengw.thread.utils.AppWrapper;
 ...
 * -Dx.block.size=2000 io.github.viscent.mtia.ch4.case02.ResponseDelayCount
 *
 * @author chengw
 * todo 没给整明白
 */
public class ResponseDelayCount {

  public static void main(String[] args) throws Exception {
    AppWrapper.invokeMain0(ResponseDelayCount.class, args, false);
  }
  ...
  private static Runnable createTask(InputStream in, int sampleInterval,
    ...
    taskClazz = null == taskClazz ? Config.CONSUMER_THREAD_CLASS_PATH
        : taskClazz;
...
//    InputStream dataIn = ResponseDelayCount.class
//        .getResourceAsStream(Config.IN_PATH);

    InputStream dataIn = null;
    Path path = Paths.get(Config.BASE_PATH + Config.IN_PATH);
    try {
      dataIn = Files.newInputStream(path);
    } catch (IOException e) {
      e.printStackTrace();
    }
    ...
    SequenceInputStream si = new SequenceInputStream(
        new Enumeration<InputStream>() {
          ...
          public InputStream nextElement() {
            ...
            if (null != fileName) {
              try {
                Path path = Paths.get(Config.BASE_PATH + Config.LOG_FILE_PATH + "/" + fileName);
