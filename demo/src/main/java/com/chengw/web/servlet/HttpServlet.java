...
/**
 *
 * 为创建一个符合某个web site 的HTTP servlet提供一个抽两类
 *
 * 其子类必须覆盖其至少一个方法：
 *  如果servlet 知识http get 请求 不许实现get方法
 *  等等
 *
 * 不必要去覆盖service 方法，service（） 用来处理标准HTTP请求，将其分发给对应的处理器方法
 *
...
public abstract class HttpServlet extends GenericServlet {
    ...
    private Method[] getAllDeclaredMethods(Class<? extends javax.servlet.http.HttpServlet> c) {

        Class<?> clazz = c;
        Method[] allMethods = null;

        while (!clazz.equals(javax.servlet.http.HttpServlet.class)) {
